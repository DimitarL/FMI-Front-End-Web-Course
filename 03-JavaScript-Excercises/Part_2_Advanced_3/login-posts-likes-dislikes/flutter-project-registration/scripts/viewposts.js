(() => {
	...
	newPost.addEventListener('submit', event => {
		...
		var input = document.getElementById("new-post-text");
		var inputValue = input.value;
		input.value = "";

		if (inputValue) {
			tweet.post(inputValue);
		}
	...
	firebase.auth().onAuthStateChanged(user => {
		...
		if (user) {
			...
			window.auth.getUserStats(user.uid).on('value', (data) => {
				document.getElementById("profile-posts-count").innerHTML = data.val().tweets;
				document.getElementById("profile-likes-count").innerHTML = data.val().likes;
			});
		}
	});
	...
	tweetsDB.on('child_changed', data => {
		...
		document.querySelector(`.like-btn[data-id=${data.key}]`).nextElementSibling.innerHTML = data.val().likes;
		document.querySelector(`.dislike-btn[data-id=${data.key}]`).nextElementSibling.innerHTML = data.val().dislikes;
	});
